  ...
  function scheduleAutoSave() {
    ...
    autoSaveTimer = setTimeout(async () => {
      try {
        ...
        console.error('Error en autoguardado completo:', e);
    ...
    if (cuatrimestreParam && centroParam) {
      ...
      if (centroPersistido && !obraForm) {
        try {
          ...
          console.error('[ERROR] Fallo al cargar centro persistido:', error);
          ui.error(`Error: El centro persistido '${centroPersistido}' ya no existe.`);
          navService.centroSeleccionado = '';
          navService.persist();
          await navService.irASeleccion();
        }
      }
